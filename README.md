...
This fork adds a Copilot integration guide and example files for using guizang-ppt-skill with Microsoft 365 Copilot (Copilot Studio, Declarative Agents).

Three integration patterns are documented:

- **Pattern A** — HTML-as-text response (zero infrastructure, simplest)
- **Pattern B** — Copilot Studio OpenAPI action connected to an external conversion server
- **Pattern C** — CodeInterpreter capability for in-session Python execution

See [`COPILOT_INTEGRATION.md`](./COPILOT_INTEGRATION.md) for the full guide (English + Korean).

Example files:

| File | Description |
|------|-------------|
| `examples/copilot-declarative-agent-manifest.json` | Declarative Agent manifest v1.4 with WebSearch + CodeInterpreter |
| `examples/copilot-openapi-action.yaml` | OpenAPI 3.0 spec for the external conversion server (`/convert` + `/render`) |

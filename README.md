...
## Microsoft 365 Copilot 사용법

이 fork 는 SKILL.md 를 그대로 두고 **루트의 `manifest.json` 하나**로 Microsoft 365 Copilot Declarative Agent 스펙(v1.4)에 맞춰 두었다. 별도의 변환 서버나 OpenAPI Action 은 필요 없다.

### Option A — Declarative Agent 로 배포 (`manifest.json` 사용)

1. `manifest.json` 을 그대로 **Microsoft 365 Agents Toolkit** (VS Code 확장) 또는 **Teams Developer Portal** 에 올린다.
2. `capabilities.OneDriveAndSharePoint.items_by_url` 의 GitHub URL 은 본인 조직 SharePoint 에 동일 파일을 복사해 두고 URL 만 바꿔 쓰면 그라운딩이 안정적이다.
3. `instructions` 필드(8000자 한도) 에는 핵심 작업 흐름과 하드룰만 담았다. 더 상세한 작성 규범이 필요하면 `SKILL.md` 와 `references/*.md` 를 Knowledge source 로 추가 등록한다.

### Option B — Copilot Studio Knowledge 로만 사용 (manifest 배포 없이)

`SKILL.md` 만 SharePoint 에 올리고 Copilot Studio 의 Knowledge 에 추가해도 동일한 동작 규칙을 따라준다. 가장 단순한 경로.

> 둘 중 어느 방식이든 generated HTML 에 `<!-- provenance: ... -->` 같은 메타 코멘트가 새지 않도록 instructions 의 *Hard rules* 가 이를 명시적으로 차단한다.
